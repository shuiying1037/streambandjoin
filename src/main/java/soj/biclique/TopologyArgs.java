...
public class TopologyArgs implements Serializable
{
    ...
    public static final String AVE_STRATEGY = "average";
    public static final String SA_STRATEGY = "sajoin";
    public static final String BAND = "band";
    public static final String LEQ = "lequi";
    public static final String GEQ = "gequi";
//在除average的其他策略中用到了
    @Option(name = "-numinstance", aliases = "--numinstance", metaVar = "<path>",
            usage = "input number of instances [def:150]")
    public int numinstance = 10;

    @Option(name = "-JoinP", aliases = "--joinpredicate", metaVar = "<path>",
            usage = "input number of instances [def:150]")
    public String joinpredicate = BAND;

    @Option(name = "-entirerange", aliases = "--entirerange", metaVar = "<path>",
            usage = "input entire range [def:100000000]")
    public int entirerange = 10000;
    ...
    @Option(name = "-e1", aliases = "--e1", metaVar = "<num>",
            usage = "# workers [def:2]")
    public long e1 = 2;

    @Option(name = "-e2", aliases = "--e2", metaVar = "<num>",
            usage = "# workers [def:2]")
    public long e2 = 2;
    ...
    @Option(name = "-ns", aliases = "--numShard", metaVar = "<num>",
            usage = "# shufflers [def:2]")
    public int numShards = 3;

    @Option(name = "-pr", aliases = "--part-r", metaVar = "<num>",
            usage = "# partitions of relation R [def:4]")
    public int numPartitionsR = 10;

    @Option(name = "-ps", aliases = "--part-s", metaVar = "<num>",
            usage = "# partitions of relation R [def:4]")
    public int numPartitionsS = 10;

    @Option(name = "-mig", aliases = "--migOrNot",
            usage = "# migOrNot [def:false]")
    public boolean migOrNot = false;

    @Option(name = "-conti", aliases = "--contiMigOrNot",
            usage = "# conMigOrNot [def:false]")
    public boolean conMigOrNot = false;

    @Option(name = "-jump", aliases = "--jumpMigOrNot",
            usage = "# conMigOrNot [def:false]")
    public boolean jumpMigOrNot = false;

    @Option(name = "-pen", aliases = "--penalAlpha", metaVar = "<num>",
            usage = "# partitions of relation S [def:4]")
    public double penalAlpha = 3.0;

    @Option(name = "-rew", aliases = "--rewardAlpha", metaVar = "<num>",
            usage = "# partitions of relation S [def:4]")
    public double rewardAlpha = 0.6;

    @Option(name = "-g", aliases = "--gatherer", metaVar = "<num>",
            usage = "# result gatherers [def:1]")
    public int numGatherers = 1;

    @Option(name = "-nps", aliases = "--numPerShard", metaVar = "<num>",
            usage = "# numPerShard [def:3]")
    public int numInitShardPerTask = 3;
    ...
    public int tuplesPerSecondR = 400; ////不能低于100，因为S流的数据比R流的数据多50倍，所以这里设置S的流速是R的50倍。

    @Option(name = "-ts", aliases = "--tps-s", metaVar = "<num>",
            usage = "tuples per second generated by each random data source"
                    + " of relation S [def:10]")
    public int tuplesPerSecondS = 20000;
    ...
    public Integer subindexSize = 256; //512; // 2048;
    ...
    @Option(name = "-i", aliases = "--interval", metaVar = "<num>",
            usage = "interval of load imbalance detection[def:" + "1000"
                    + "]")
    public double interval = 1000;//5000;
...
//                "Non-negative number of sources S is required");
    ...
    private void deriveArgs() {
        ...
        winR = (long)winInSecR; ///(winInSecR * 1000);
        winS = (long)winInSecS; ///winInSecS * 1000);
    }
