...
public class PostProcessBolt extends BaseRichBolt
{
    ...
    public void prepare(Map conf, TopologyContext context,
        ...
        String prefix = "post" + context.getThisTaskId();
//        _output = new FileWriter("/tmp/", prefix, "csv");
    ...
    public void execute(Tuple tuple) {
        Values v = new Values(tuple.getLongByField("currentMoment"),
                tuple.getLongByField("tuplesStored"), tuple.getLongByField("joinTimes"),
                tuple.getLongByField("processingDuration"), tuple.getDoubleByField("latency"),
                tuple.getLongByField("migrationTime"), tuple.getLongByField("tuplesJoined"), tuple.getLongByField("resultNum"),
                tuple.getLongByField("load"));
...
//        output("_statistics.size() = " + _statistics.size() + ",_boltsNum=" + _boltsNum +  "\n");
                    ...
                    if(equals){
                        ...
                        for (Map.Entry<Integer, Queue<Values>> e : _statistics.entrySet()) {
                            if(!e.getValue().isEmpty()){
                                ...
                                tuplesStored += getLong(temp.get(1));
                                ...
                                tuplesJoined += getLong(temp.get(6));
                                resultNum += getLong(temp.get(7));
                                num++;
                                load += getLong(temp.get(8));
//                                output(" "+load);
                            }
                        }
                        for(Integer i : lst){
                        ...
                        sb.append("@ [" + ts + " sec]");
                        double throughput = tuplesStored;
                        ...
                        sb.append(String.format("throughput-Stored, %d", tuplesStored));
                        sb.append(String.format(", joined tuple, %d", tuplesJoined));
                        sb.append(String.format(", latency before div, %f", latency));
                        if(tuplesJoined == 0){
                            latency = 0;
                        }
                        else{
//                            latency /= tuples;
                            latency /= tuplesJoined;
                        }
